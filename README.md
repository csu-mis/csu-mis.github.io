...
- 部署份數以 GitHub 文件庫為準：`組織章程.md`（或舊檔名 `rules.md`）、`辦法/*.md`、`作業規範/*.md` 有幾份就公開幾份。README 與 `修訂說明/` 不公開為閱讀頁。
- `src/data/documents.json` 只覆寫顯示名稱、簡介與穩定網址；沒寫進清單的新文件仍會自動出現。正文一律由 GitHub 讀取。
